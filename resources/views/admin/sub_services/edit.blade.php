        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div class="flex items-center gap-6">
                ...
                <div>
                    ...
                    <p class="text-slate-500 mt-1.5 font-medium italic">Modify the operational parameters for the "{ $subService->name }" module.</p>
                </div>
            </div>
        </div>
    </x-slot>
                ...
                <div class="relative z-10">
                    <div class="mb-16">
                        <div class="flex items-center gap-5 mb-4">
                            ...
                            <h3 class="text-3xl font-black text-slate-900 tracking-tight">Module Optimization</h3>
                        </div>
                        <p class="text-slate-500 text-lg font-bold leading-relaxed italic border-l-4 border-amber-600 pl-6">Fine-tune the technical specifications and valuation for this service entity. Changes will propagate to active provider nodes.</p>
                    </div>

                    <form action="{{ route('admin.sub-services.update', $subService) }}" method="POST" class="space-y-12">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                            <div class="group">
                                <label for="category_id" class="flex items-center justify-between mb-4">
                                    ...
                                    <span class="text-[9px] font-black text-slate-300 italic uppercase tracking-widest">Parent Node Active</span>
                                </label>
                                <div class="relative">
                                    <select id="category_id" name="category_id" required 
                                        ...
                                        @foreach($categories as $category)
                                            <option value="{{ $category->id }}" {{ old('category_id', $subService->category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }} INFRA</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="group">
                                ...
                                <div class="relative">
                                    <input id="name" type="text" name="name" value="{{ old('name', $subService->name) }}" required 
                        ...
                        <div class="group">
                            <label for="description" class="flex items-center justify-between mb-4">
                                ...
                                <span class="text-[9px] font-black text-slate-300 italic uppercase tracking-widest">Client Interaction</span>
                            </label>
                            <textarea id="description" name="description" rows="5" required 
                                class="block w-full p-8 bg-slate-50 border-2 border-transparent rounded-[2.5rem] text-lg font-bold text-slate-600 focus:ring-0 focus:border-indigo-600 focus:bg-white transition-all shadow-inner leading-relaxed"
                                placeholder="Detail the technical fulfillment steps and expected service quality markers...">{{ old('description', $subService->description) }}</textarea>
                        ...
                        <div class="group md:w-1/2">
                            <label for="price" class="flex items-center justify-between mb-4">
                                ...
                                <span class="text-[9px] font-black text-slate-300 italic uppercase tracking-widest">Market Standard</span>
                            </label>
                            <div class="relative">
                                <input id="price" type="number" step="1" name="price" value="{{ old('price', $subService->price) }}" required 
                                    ...
                                    placeholder="499">
                                <span class="absolute left-8 top-1/2 -translate-y-1/2 font-black text-amber-300 text-3xl italic">₹</span>
                            </div>
                        ...
                        <div class="p-8 bg-slate-900 rounded-[2.5rem] text-white flex flex-col md:flex-row items-center justify-between shadow-2xl mt-16 relative overflow-hidden group/btn gap-8">
                            <div class="relative z-10 flex items-center gap-6">
                                <div class="w-16 h-16 bg-white/10 rounded-2xl flex items-center justify-center text-3xl text-amber-400 shadow-inner group-hover/btn:rotate-12 transition-transform duration-500 italic font-black">⚡</div>
                                <div>
                                    <p class="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 italic mb-1">State: PARAM_MODIFIED</p>
                                    <p class="text-lg font-black italic tracking-tight leading-none">Confirm Optimization Seal</p>
                                </div>
                            </div>
                            
                            <div class="flex items-center gap-6 relative z-10 w-full md:w-auto">
                                <a href="{{ route('admin.sub-services.index') }}" class="text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 hover:text-white transition-colors px-6 py-4">Discard</a>
                            ...
                            <div class="absolute -right-6 top-1/2 -translate-y-1/2 text-[10rem] opacity-[0.03] group-hover/btn:scale-125 transition-transform duration-700 italic font-black pointer-events-none uppercase">UPDT</div>
                        </div>
                    </form>
                </div>
            </div>
